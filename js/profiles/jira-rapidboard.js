...
(function() {
  var addButton, addButtons, config, debug, findAccountId, findDescription, findIdsForItem, findItemId, findProjectId, findProjectName, insertScript, isConfigLoaded, isDataSufficient, isItemComplete, itemSelector, listenForPlatformReady, loadConfig, namespacedString, nsEvent, nsId, projectNameSelector, sendMessage, itemDescriptionSelector;

  projectNameSelector = '.ghx-project';
  itemSelector = "#ghx-detail-issue";
  itemDescriptionSelector = "dd[data-field-id=summary]";

  debug = false;

  isConfigLoaded = false;

  config = "window._harvestPlatformConfig = {    'applicationName': 'Jira',    'permalink': 'https://%ACCOUNT_ID%.jira.com/browse/%ITEM_ID%',    'debug': " + debug + "  }";
  ...
  insertScript = function(callback) {
    ...
    var styles = '.harvest-timer.styled{border-radius:3px; display:block; margin:0 auto; text-align: center; padding: 5px; margin-bottom: 10px;}';
  ...
  findProjectName = function(item) {
    return item.find(projectNameSelector).text();
  };

  findDescription = function(item) {
      var id, description;
      id = findItemId(item);
      description = item.find(itemDescriptionSelector).text();
  ...
  addButtons = function() {
    ...
    return items.each(function() {
      ...
      if ($(this).find('.harvest-timer').length === 0) {
        ...
        ids['project']['name'] = findProjectName($(this));
  ...
  setInterval(function() {
    var href, pattern;
    pattern = /^http[s]?:\/\/(\S+.)jira.com\/secure\/RapidBoard.*$/;
  ...
  addButton = function(element, data) {
    data['element'] = $(element).find('.ghx-detail-nav-menu');
    return sendMessage({
      'type': 'timers:request',
      'data': 'data',
      data: data
